      ...
      <?php
            if ($result) {
                foreach ($result as $serviceData) {
                    ...
                    echo '<button class="btn btn-danger" onclick="deleteService(' . (isset($serviceData['user_id']) ? $serviceData['user_id'] : '') . ')">Delete</button>';
                    echo '</div>';
                    echo '</div>';
                    echo '</div>';
                }
...
<script>
  ...
  <a href="csprocess3.php"><button class="btn btn-primary btn-md">Add services</button></a>
